...
namespace CourseWork.ViewModel
{
    public class ReservationVM : BaseVM
    {
        ...
        private void _AddNewReservation()
        {
            ...
            if (Math.Abs(TimeFrom.TotalMinutes - TimeTo.TotalMinutes) > 30 & !isIntersect)
            {
                ReservationService.CreateReservation(FillUtil.ROW,
                                                     ...
                                                     TimeFrom,
                                                     TimeTo);
                MessageBox.Show("Данные заполнены, теперь можно закрыть окно", "Success!", MessageBoxButton.OK, MessageBoxImage.Asterisk);
            }
            else MessageBox.Show("Время задано неверно или занято", "Error!", MessageBoxButton.OK, MessageBoxImage.Error);
        }
        ...
        {
            MessageBoxResult result = MessageBox.Show("Вы уверены?", "Warning!", MessageBoxButton.YesNo, MessageBoxImage.Question);
            if (result == MessageBoxResult.Yes)
            {
                ReservationService.DeleteReservation(FillUtil.ROW,
                                       ...
                                       FillUtil.PAGE,
                                       AuthVM.Nickname);
                MessageBox.Show("Данные удалены, теперь можно закрыть окно", "Success!", MessageBoxButton.OK, MessageBoxImage.Asterisk);
            }
        }
