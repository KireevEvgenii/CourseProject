            ...
            for (int row = 0; row < 24; row++)
            {
                ...
                for (int column = 0; column < 7; column++)
                {
                        if (ReservationService.isReservationExist(row, column, page, AuthVM.Nickname))
                        {
                            ...
                            ToolTip toolTip = new ToolTip() { Content = ReservationService.GetReservationInfo(row, column, page, AuthVM.Nickname).Members + "\n" + ReservationService.GetReservationInfo(row, column, page, AuthVM.Nickname).TimeFrom + "\n" + ReservationService.GetReservationInfo(row, column, page, AuthVM.Nickname).TimeTo, FontSize = 12 };
                            ((Button)ui).ToolTip = toolTip;
                        }
                        else
                        {
